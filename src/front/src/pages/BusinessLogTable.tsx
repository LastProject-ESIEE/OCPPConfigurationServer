...
export function BusinessLogTable() {
    ...
    let props: InfinityScrollItemsTableProps<BusinessLog> = {
        ...
        hasMore: hasMore,
        error: error,
        onSelection: businessLog => { console.log("Selected item : " + businessLog.id) },
                        ...
                        <ListItemButton style={{maxWidth: "true", height:"5vh", padding: 0, paddingTop: 3, borderRadius: 50, color: 'black', backgroundColor: '#E1E1E1'}}>
                            <Grid container maxWidth={"true"} flexDirection={"row"} alignItems={"center"}>
                                <Grid item xs={12/businessLogTableColumns.length} maxWidth={"true"} justifyContent={"center"}>
                                    <Typography variant="body1" align="center">{new Date(businessLog.date).toLocaleString()}</Typography>
                                </Grid>
                                <Grid item xs={12/businessLogTableColumns.length} maxWidth={"true"} justifyContent={"center"}>
                                ...
                                <Grid item xs={12/businessLogTableColumns.length} maxWidth={"true"} justifyContent={"center"}>
                                    <Typography variant="body1" align="center">{businessLog.category}</Typography>
                                </Grid>
                                <Grid item xs={12/businessLogTableColumns.length} maxWidth={"true"} justifyContent={"center"}>
                                    <Typography variant="body1" align="center">{businessLog.completeLog}</Typography>
                                </Grid>
                            </Grid>
                        </ListItemButton>
                    {/*</Link>*/}
